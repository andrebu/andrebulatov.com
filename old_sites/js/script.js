...
function applyNavUpDownBtns() {
	...
	function nextSection() {
		if (!$('body.fixed-page').length) $('.scroll-down-page').click();
		if ($('.nav li.active').next().hasClass('scroller')) return;
		$('.nav li.active').next().find('a').click();
	}
	upBtn.on('click', function(e) { e.preventDefault(); prevSection(); });
	downBtn.on('click', function(e) { e.preventDefault(); nextSection(); });
...
// quantum 
function applyNewInputSty() {
	var aN = $('.animated-input input');
	aN.focus(function(){
		$(this).parent().addClass('isFocused');
	}).blur(function(){
		$(this).parent().removeClass('isFocused');
	});
	aN.on('keyup', function(e){
		if ($(this).val() != '') {
